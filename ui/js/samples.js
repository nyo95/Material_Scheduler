window.Samples = (function(){
  ...
  function render(){
    ...
    el.innerHTML = `
      <div class="toolbar"><div class="muted">${rows.length} samples</div></div>
  ...
  function wire(){
    document.querySelectorAll('#s_rows tr').forEach(tr=>{
      ...
      const notes=tr.querySelector('.s_notes'); if(notes){ notes.addEventListener('change',()=>{ __rpc('set_flags',{ ids:[id], flags:{ sample_notes: notes.value } }); }); }
